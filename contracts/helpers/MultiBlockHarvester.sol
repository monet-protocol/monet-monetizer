...
contract MultiBlockHarvester is BaseHarvester {
  ...
  function finalizeRebalance(address yieldBearingAsset, uint256 balance) external onlyTrusted {
    try monetizer.updateOracle(yieldBearingAsset) { } catch { }
    _adjustAllowance(yieldBearingAsset, address(monetizer), balance);
    uint256 amountOut =
      monetizer.swapExactInput(balance, 0, yieldBearingAsset, address(tokenP), address(this), block.timestamp);
    ...
    internal
  {
    ...
    if (typeAction == 1) {
      uint256 amountOut =
        monetizer.swapExactInput(amount, 0, address(tokenP), yieldBearingInfo.asset, address(this), block.timestamp);
      if (yieldBearingAsset == XEVT) {
        ...
        _adjustAllowance(yieldBearingAsset, address(monetizer), shares);
        amountOut =
          monetizer.swapExactInput(shares, 0, yieldBearingAsset, address(tokenP), address(this), block.timestamp);
      ...
      uint256 amountOut =
        monetizer.swapExactInput(amount, 0, address(tokenP), yieldBearingAsset, address(this), block.timestamp);
