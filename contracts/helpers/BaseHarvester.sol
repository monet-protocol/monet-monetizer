...
struct YieldBearingParams {
  ...
  // Maximum exposure within the Monetizer to the deposit asset
  uint64 maxExposure;
  // Minimum exposure within the Monetizer to the deposit asset
  uint64 minExposure;
  // Whether limit exposures should be overriden or read onchain through the Monetizer
  // This value should be 1 to override exposures or 2 if these shouldn't be overriden
  uint64 overrideExposures;
  // Maximum slippage when dealing with the Monetizer
  uint96 maxSlippage;
}
...
/// @dev Abstract contract for a harvester that aims at rebalancing a Monetizer
...
abstract contract BaseHarvester is IHarvester, AccessManaged {
  ...
  /// @notice Reference to the `monetizer` implementation this contract aims at rebalancing
  IMonetizer public immutable monetizer;
  /// @notice TokenP handled by the `monetizer` of interest
  ITokenP public immutable tokenP;
  /// @notice Max slippage when dealing with the Monetizer
  ...
  constructor(
    ...
    IMonetizer definitiveMonetizer
  )
    AccessManaged(initialAuthority)
  {
    tokenP = definitiveTokenP;
    monetizer = definitiveMonetizer;
  }
   ...
   * @param minExposure minimum exposure within the Monetizer to the asset
   * @param maxExposure maximum exposure within the Monetizer to the asset
   * @param overrideExposures whether limit exposures should be overriden or read onchain through the Monetizer
   */
  ...
  /**
   * @notice Compute the amount needed to rebalance the Monetizer
   * @param yieldBearingAsset address of the yield bearing asset
    ...
    returns (uint8 increase, uint256 amount)
  {
    (uint256 stablecoinsFromYieldBearingAsset, uint256 stablecoinsIssued) =
      monetizer.getIssuedByCollateral(yieldBearingAsset);
    (uint256 stablecoinsFromAsset,) = monetizer.getIssuedByCollateral(yieldBearingInfo.asset);
    ...
    virtual
  {
    ...
    (xFeeMint,) = monetizer.getCollateralMintFees(asset);
    ...
    (xFeeBurn,) = monetizer.getCollateralBurnFees(asset);
