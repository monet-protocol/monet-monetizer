...
/// @dev Generic contract for anyone to permissionlessly adjust the reserves of Angle Monetizer
...
contract GenericHarvester is BaseHarvester, IERC3156FlashBorrower, RouterSwapper {
  ...
  /// @notice Monet stablecoin flashloan contract
  ...
  constructor(
    ...
    BaseHarvester(initialAuthority, definitivetokenP, definitiveMonetizer)
  {
  ...
  /// @dev This transaction either reduces the exposure to `yieldBearingAsset` in the Monetizer or frees up
  /// some yieldBearingAsset that can then be used for people looking to burn deposit tokens
  /// @dev Due to potential transaction fees within the Monetizer, this function doesn't exactly bring
  ...
  function harvest(address yieldBearingAsset, uint256 scale, bytes calldata extraData) public virtual {
    ...
    try monetizer.updateOracle(yieldBearingAsset) { } catch { }
    ...
    returns (bytes32)
  {
    ...
    uint256 amountOut = monetizer.swapExactInput(amount, 0, address(tokenP), tokenIn, address(this), block.timestamp);

    // Swap to tokenIn
    amountOut = _swapToTokenOut(typeAction, tokenIn, tokenOut, amountOut, swapType, callData);

    _adjustAllowance(tokenOut, address(monetizer), amountOut);
    uint256 amountStableOut =
      monetizer.swapExactInput(amountOut, minAmountOut, tokenOut, address(tokenP), address(this), block.timestamp);
