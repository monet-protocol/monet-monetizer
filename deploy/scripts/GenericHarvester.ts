...
export default deployScript(
  async ({ namedAccounts, network, deploy, get }) => {
    ...
    const monetizer = get(`Monetizer_${token}`);
    if (!monetizer) {
      throw new Error(`Monetizer_${token} not found`);
    }

    const genericHarvester = await deploy(`${contractName}_${token}`, {
      account: deployer,
      artifact: artifacts.GenericHarvester,
      args: [tokenTransferAddress, swapRouter, tokenP, monetizer.address, accessManager, flashloan],
    });
