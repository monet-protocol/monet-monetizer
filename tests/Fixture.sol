...
contract Fixture is Monetizer, ConfigAccessManager {
  ...
  function _mintExactOutput(address owner, address tokenIn, uint256 amountStable, uint256 estimatedAmountIn) internal {
    ...
    IERC20(tokenIn).approve(address(monetizer), type(uint256).max);
    monetizer.swapExactOutput(amountStable, estimatedAmountIn, tokenIn, address(tokenP), owner, block.timestamp * 2);
    vm.stopPrank();
  }
  ...
  function _mintExactInput(address owner, address tokenIn, uint256 amountIn, uint256 estimatedStable) internal {
    ...
    IERC20(tokenIn).approve(address(monetizer), type(uint256).max);
    monetizer.swapExactInput(amountIn, estimatedStable, tokenIn, address(tokenP), owner, block.timestamp * 2);
    vm.stopPrank();
  }
}
