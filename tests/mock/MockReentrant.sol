...
contract ReentrantRedeemGetCollateralRatio {
  ...
  constructor(IMonetizer _transmuter, IERC1820Registry _registry) {
    monetizer = _transmuter;
    registry = _registry;
  }

  function testERC777Reentrancy(uint256 redeemAmount) public {
    uint256[] memory minAmountOuts;
    (, uint256[] memory quoteAmounts) = monetizer.quoteRedemptionCurve(redeemAmount);
    minAmountOuts = new uint256[](quoteAmounts.length);
    monetizer.redeem(redeemAmount, address(this), block.timestamp * 2, minAmountOuts);
  }
...
contract ReentrantRedeemSwap {
  ...
  constructor(IMonetizer _transmuter, IERC1820Registry _registry, IERC20 _tokenP, IERC20 _collateral) {
    monetizer = _transmuter;
    registry = _registry;
    tokenP = _tokenP;
  ...
  function tokensReceived(address, address from, address, uint256, bytes calldata, bytes calldata) external {
    ...
    if (from != address(0)) {
      // It should revert here
      monetizer.swapExactInput(1e18, 0, address(collateral), address(tokenP), address(this), block.timestamp * 2);
    }
  }
