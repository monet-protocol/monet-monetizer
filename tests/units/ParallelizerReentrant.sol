...
import { CollateralSetup, Test } from "contracts/monetizer/configs/Test.sol";
...
import { Monetizer } from "../utils/Monetizer.sol";
import { ConfigAccessManager } from "../utils/ConfigAccessManager.sol";

contract MonetizerReentrantTest is Monetizer, ConfigAccessManager {
  ...
  function setUp() public {
    ...
    accessManager.setTargetFunctionRole(address(monetizer), getMonetizerGovernorSelectorAccess(), GOVERNOR_ROLE);
    accessManager.setTargetFunctionRole(address(monetizer), getMonetizerGuardianSelectorAccess(), GUARDIAN_ROLE);
    vm.stopPrank();

    vm.label(address(tokenP), "tokenP");
    vm.label(address(monetizer), "Monetizer");
    ...
    monetizer.setFees(address(eurA), xFeeMint, yFee, true);
    monetizer.setFees(address(eurA), xFeeBurn, yFee, false);
    monetizer.setFees(address(eurB), xFeeMint, yFee, true);
    monetizer.setFees(address(eurB), xFeeBurn, yFee, false);
    monetizer.setFees(address(eurY), xFeeMint, yFee, true);
    monetizer.setFees(address(eurY), xFeeBurn, yFee, false);
    monetizer.setRedemptionCurveParams(xFeeMint, yFeeRedemption);
    vm.stopPrank();
    ...
    contractReentrantRedeemGetCollateralRatio =
      new ReentrantRedeemGetCollateralRatio(IMonetizer(address(monetizer)), IERC1820Registry(address(registry)));
    contractReentrantRedeemGetCollateralRatio.setInterfaceImplementer();

    contractReentrantRedeemSwap = new ReentrantRedeemSwap(
      IMonetizer(address(monetizer)),
  ...
  function testFuzz_ReentrantRedeemCollateralRatio(uint256[3] memory initialAmounts) public {
    ...
    (, uint256[] memory quoteAmounts) = monetizer.quoteRedemptionCurve(amountBurnt);
    ...
    returns (uint256 mintedStables, uint256[] memory collateralMintedStables)
  {
    ...
    for (uint256 i; i < _collaterals.length; ++i) {
      ...
      IERC20(_collaterals[i]).approve(address(monetizer), initialAmounts[i]);

      collateralMintedStables[i] =
        monetizer.swapExactInput(initialAmounts[i], 0, _collaterals[i], address(tokenP), alice, block.timestamp * 2);
