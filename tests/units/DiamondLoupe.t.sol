...
contract Test_DiamondLoupe is Fixture {
  ...
  function test_FacetFunctionSelectors() public {
    ...
    hoax(governor);
    monetizer.diamondCut(facetCut, address(0), ""); // Deploy only the first selector

    bytes4[] memory fetchedSelectors = monetizer.facetFunctionSelectors(pureFacet);
    ...
    hoax(governor);
    monetizer.diamondCut(facetCut, address(0), ""); // Deploy the second selector

    fetchedSelectors = monetizer.facetFunctionSelectors(pureFacet);
  ...
  function test_FacetAddresses() public {
    address[] memory facetAddresses = monetizer.facetAddresses();
  ...
  function test_FacetAddress() public {
    for (uint256 i; i < facetNames.length; ++i) {
      ...
      for (uint256 j; j < selectors.length; ++j) {
        assertEq(monetizer.facetAddress(selectors[j]), facetAddressList[i]); // Check all selectors are
          // present
      }
    }
  }
