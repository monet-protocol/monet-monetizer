...
import { Layout } from "contracts/monetizer/Layout.sol";
...
contract Test_Layout is Fixture {
  ...
  function setUp() public override {
    super.setUp();
    layout = Layout(address(monetizer));
  }

  function test_Layout() public {
    address tokenP = address(monetizer.tokenP());
    uint8 isRedemptionLive = monetizer.isPaused(address(0), ActionType.Redeem) ? 0 : 1;
    uint256 stablecoinsIssued = monetizer.getTotalIssued();
    address[] memory collateralList = monetizer.getCollateralList();
    (uint64[] memory xRedemptionCurve, int64[] memory yRedemptionCurve) = monetizer.getRedemptionFees();
    Collateral memory collateral = monetizer.getCollateralInfo(collateralList[0]);
    hoax(governor);
    monetizer.toggleTrusted(alice, TrustedType.Updater);
    hoax(governor);
    monetizer.toggleTrusted(alice, TrustedType.Seller);
    address accessManager = monetizer.accessManager();
    hoax(guardian);
    monetizer.setDummyImplementation(address(alice));
    address implementation = monetizer.implementation();
