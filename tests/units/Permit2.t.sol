...
contract Permit2Test is Fixture, FunctionUtils {
  ...
  function test_RevertWhen_SwapExactInputWithPermit_InvalidSigner() public {
    ...
    bytes memory sig = getPermitTransferSignature(permit, 2, DOMAIN_SEPARATOR, address(monetizer));

    vm.expectRevert(SignatureVerification.InvalidSigner.selector);
    startHoax(alice);
    monetizer.swapExactInputWithPermit(BASE_6, 0, address(eurA), alice, deadline, abi.encode(nonce, sig));
  }
  ...
  function test_RevertWhen_SwapExactInputWithPermit_InvalidSender() public {
    ...
    bytes memory sig = getPermitTransferSignature(permit, 1, DOMAIN_SEPARATOR, address(monetizer));
  ...
  function test_RevertWhen_SwapExactInputWithPermit_InvalidNonce() public {
    ...
    vm.expectRevert(SignatureVerification.InvalidSigner.selector);
    startHoax(bob);
    monetizer.swapExactInputWithPermit(BASE_6, 0, address(eurA), alice, deadline, abi.encode(nonce + 1, sig));
  }
  ...
  function test_RevertWhen_SwapExactOutputWithPermit_InvalidAmount() public {
    ...
    vm.expectRevert(SignatureVerification.InvalidSigner.selector);
    startHoax(alice);
    monetizer.swapExactOutputWithPermit(amountOut, amountInMax, address(eurA), alice, deadline, abi.encode(nonce, sig));
  }
  ...
  function test_SwapExactInputWithPermit() public {
    ...
    assertEq(eurA.balanceOf(address(monetizer)), BASE_6);
  }
  ...
  function test_SwapExactOutputWithPermit() public {
    ...
    assertEq(eurA.balanceOf(address(monetizer)), amountIn);
  }
