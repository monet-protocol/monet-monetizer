...
contract PathIndeInvariants is Fixture {
  ...
  function setUp() public virtual override {
    ...
    _traderHandler = new TraderWithSplit(monetizer, transmuterSplit, _collaterals, _oracles, _NUM_TRADER);
    _arbitragerHandler = new ArbitragerWithSplit(monetizer, transmuterSplit, _collaterals, _oracles, _NUM_ARB);
  ...
  function invariant_PathIndependenceSubSupply() public {
    for (uint256 i; i < _collaterals.length; i++) {
      (uint256 issuedPerCollat,) = monetizer.getIssuedByCollateral(_collaterals[i]);
  ...
  function invariant_PathIndependenceBalanceCollaterals() public {
    for (uint256 i; i < _collaterals.length; i++) {
      uint256 balance = IERC20(_collaterals[i]).balanceOf(address(monetizer));
  ...
  function invariant_PathIndependenceCollateralRatio() public {
    (uint256 collateralRatio,) = monetizer.getCollateralRatio();
