...
import { CollateralSetup, Fixture, IMonetizer, Test } from "../Fixture.sol";
...
contract BasicInvariants is Fixture {
  ...
  function setUp() public virtual override {
    ...
    // Deploy another monetizer to check the independant path property
    config = address(new Test());
    transmuterSplit = deployReplicaMonetizer(
      config,
    ...
    accessManager.setTargetFunctionRole(
      address(transmuterSplit), getMonetizerGovernorSelectorAccess(), GOVERNOR_ROLE
    );
    accessManager.setTargetFunctionRole(
      address(transmuterSplit), getMonetizerGuardianSelectorAccess(), GUARDIAN_ROLE
    );
    vm.stopPrank();

    {
      ...
      monetizer.setRedemptionCurveParams(xFeeRedemption, yFeeRedemption);
    ...
    _traderHandler = new Trader(monetizer, transmuterSplit, _collaterals, _oracles, _NUM_TRADER);
    _arbitragerHandler = new Arbitrager(monetizer, transmuterSplit, _collaterals, _oracles, _NUM_ARB);
    _governanceHandler = new Governance(monetizer, transmuterSplit, _collaterals, _oracles);
    ...
    vm.label(address(transmuterSplit), "MonetizerSplit");
  ...
  function systemState() public view {
    ...
    (uint256 issuedA, uint256 issued) = monetizer.getIssuedByCollateral(address(eurA));
    (uint256 issuedB,) = monetizer.getIssuedByCollateral(address(eurB));
    (uint256 issuedY,) = monetizer.getIssuedByCollateral(address(eurY));
  ...
  function invariant_Supply() public {
    uint256 stablecoinIssued = monetizer.getTotalIssued();
  ...
  function invariant_CollateralRatio() public {
    ...
    (uint64 collateralRatio,) = monetizer.getCollateralRatio();
  ...
  function invariant_MintReflexivity() public {
    ...
    for (uint256 i; i < _collaterals.length; i++) {
      uint256 amountIn = monetizer.quoteOut(stableAmount, _collaterals[i], address(tokenP));
      uint256 reflexiveStableAmount = monetizer.quoteIn(amountIn, _collaterals[i], address(tokenP));
  ...
  function invariant_BurnReflexivity() public {
    ...
    for (uint256 i; i < _collaterals.length; i++) {
      try monetizer.quoteIn(stableAmount, address(tokenP), _collaterals[i]) returns (uint256 amountOut) {
        uint256 reflexiveStableAmount = monetizer.quoteOut(amountOut, address(tokenP), _collaterals[i]);
