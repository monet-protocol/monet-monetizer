...
import { AggregatorV3Interface, BaseActor, IERC20, IERC20Metadata, IMonetizer, TestStorage } from "./BaseActor.t.sol";
import { QuoteType } from "contracts/monetizer/Storage.sol";
...
contract ArbitragerWithSplit is BaseActor {
  ...
  constructor(
    ...
    BaseActor(nbrTrader, "Arb", monetizer, transmuterSplit, collaterals, oracles)
  { }
    ...
    returns (uint256, uint256)
  {
    ...
    if (quoteType == QuoteType.MintExactInput) {
      ...
      // divided by 2 because we need to do for both the monetizer and the replica
    ...
    if (quoteType == QuoteType.MintExactInput || quoteType == QuoteType.BurnExactInput) {
      ...
      // replicate on the other monetizer but split the orders
      {
      ...
      // replicate on the other monetizer but wplit the orders
      {
    ...
    countCall("redeem")
  {
    ...
    // Redeem on the replica monetizer
    {
      uint256[] memory balanceTokens = new uint256[](_collaterals.length);
      for (uint256 i; i < balanceTokens.length; ++i) {
        balanceTokens[i] = IERC20(_collaterals[i]).balanceOf(_currentActor);
