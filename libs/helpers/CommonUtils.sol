...
contract CommonUtils is CommonBase, JsonReader {
  ...
  function _getConnectedChains(string memory token) internal returns (uint256[] memory, address[] memory) {
    ...
    for (uint256 i = 0; i < allChainIds.length; i++) {
      ...
      if (keccak256(abi.encodePacked(token)) == EUR_HASH) {
        try this.chainToContract(allChainIds[i], ContractType.LZEURmo) returns (address _addr) {
          addr = _addr;
        } catch { }
      } else if (keccak256(abi.encodePacked(token)) == USD_HASH) {
        try this.chainToContract(allChainIds[i], ContractType.LZUSDmo) returns (address _addr) {
          addr = _addr;
        } catch { }
      }
  ...
  function _getContractPath(ContractType name) internal pure returns (string memory) {
    if (name == ContractType.EURmo) {
      return "EUR.token";
    } else if (name == ContractType.USDmo) {
      return "USD.token";
    } else if (name == ContractType.LZEURmo) {
      return "EUR.lzToken";
    } else if (name == ContractType.LZUSDmo) {
      return "USD.lzToken";
    } else if (name == ContractType.PRL) {
      return "PRL";
    } else if (name == ContractType.sEURmo) {
      return "EUR.Savings";
    } else if (name == ContractType.sUSDmo) {
      return "USD.Savings";
    } else if (name == ContractType.Timelock) {
      return "Timelock";
    } else if (name == ContractType.MonetizerEURmo) {
      return "EUR.Monetizer";
    } else if (name == ContractType.MonetizerUSDmo) {
      return "USD.Monetizer";
    } else if (name == ContractType.TreasuryEURmo) {
      return "EUR.Treasury";
    } else if (name == ContractType.TreasuryUSDmo) {
      return "USD.Treasury";
